...
topicSchema.methods.findMatch = function(message, user, plugins, scope, callback) {
  ...
  eachGambit = function() {
    async.each(that.gambits, eachGambitHandle, function eachGambitHandleComplete(){
    ...
    regexreply.postParse(trigger.trigger, message, user, function(regexp) {
      ...
      if (match) {
        if (trigger.filter === true) {
          ...
          debug("Updating Message Object with Trigger Match", message.triggerId)
          matches.push({stars:stars, trigger: trigger, topic: that.name});
          callback();
        }
      } else {
