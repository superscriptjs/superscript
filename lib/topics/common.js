            ...
            if (trigger.isQuestion && message.isQuestion) {
                ...
                if (trigger.isDisordered === true) {
                    ...
                    if (intersection.length > 0) {
                        ...
                        var etalon2 = trigger.trigger.match(/\\s\*\(\(\?\:\\s\+\\\(\?\\~\?\[\\wА-я-\:\]\+\[\\\?\\\.\\'\\,\\~\\\)\\w\]\*\?\)\{\d,\d\}\)/);

                        for (i = 0; i < starPatterns.length; i++) {
                            //if (!isStars) 
                            isStars = trigger.trigger.match(starPatterns[i]);
                        }
                        
                        if (isStars) {
                            //console.log('#--disordered with stars trigger.trigger', trigger.trigger + '|');

                            var txtOnlyPattern = trigger.trigger.replace(/\(\\S\+\(\?\:\\s\+\\S\+\)\{\d\}\)/g, '').trim();
                              //console.log('#txtOnlyPattern', txtOnlyPattern);
                            var txtOnlyPatternArray = txtOnlyPattern.split(' ');
                              //console.log('#txtOnlyPatternArray', txtOnlyPatternArray);
                            ...
                            starsOnlyMessage = starsOnlyMessage.join(' ');
                              //console.log('# starsOnlyMessage', starsOnlyMessage);

                            var starsOnlyPattern = trigger.trigger.replace(txtOnlyPattern, '').trim();
                              //console.log('#starsOnlyPattern', starsOnlyPattern);
                            ...
                            if (matchedStarsArray) {
                              ...
                              //console.log('#matchedStarsArray', matchedStarsArray);
                            
                              //console.log('# matchedStarsArrayJoined', matchedStarsArrayJoined);
                            var txtOnlyMessageArray = messageArray.filter(function(val) {
                              ...
                              //console.log('#txtOnlyMessageArray', txtOnlyMessageArray);
