            ...
            if (trigger.isQuestion && message.isQuestion) {
                ...
                if (trigger.isDisordered === true) {
                    ...
                    if (optionals) {
                        ...
                        ruleArray = trigger.trigger.replace(optionalsPatternRx, '').trim().split(' ');
                        messageArray = messageWithoutOptionals || messageArray;
                    };
                    ...
                    if (intersection.length > 0) {
                        ...
                        if (allAlternates) {
                            ...
                            for (var i = 0, j = 0; i < ruleArray.length; i++) {
                                if(!ruleArray[i].match(alternatePatternRx)) {
                        ...
                        // TODO тут правило без альтернатив сбрасывается. так как это ссылка на ячейку в памяти
                        if (match) ruleArray = ruleWittoutAlternates;
                        for (var i = 0; i < ruleArray.length; i++) {
                            for (var j = 0, f = false; j < starPatternsArray.length; j++) {
                                if (ruleArray[i].match(starPatternsArray[j])) {
                                    foundStarPatterns.push(ruleArray[i]);
                                    f = true;
                                    break;
                                }
                            } 
                        ...
                        if (foundStarPatterns.length > 0 && match) {
                            ...
                            var messageStarsOnly = messageWithoutAlternates || messageArray;
                        ...
                        if (match) {
                            if (match.length > 1) {
                                for (var i = 1; i < match.length; i++) {
                                    ruleArray.push(match[i].split(' '))
                                    console.log('-', i, ruleArray)
                                }
                            }
                            console.log("--", pattern)
