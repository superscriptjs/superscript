...
module.exports = function(mongoose) {
    ...
    var _afterHandle = function (match, matches, trigger, topic, cb) {
        ...
        debug.info("Match found '%s' in topic %s'", trigger.input, topic)
        var stars = [];
        if (match.length > 1) {
    ...
    var _doesMatch = function(trigger, message, user, callback) {
        ...
        postParse.postParse(trigger.trigger, message, user, function complexFunction(regexp) {
            ...
            debug.verbose("Try to match (clean)'%s' against %s (%s)", message.clean, trigger.trigger, regexp)
            debug.verbose("Try to match (lemma)'%s' against %s (%s)", message.lemString, trigger.trigger, regexp)
