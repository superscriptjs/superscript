...
module.exports = function (mongoose, facts) {
  ...
  **/
  ...
  var gambitSchema = new mongoose.Schema({
    ...
    // If the trigger is a Disordered Match
    isDisordered: {type: Boolean, default: false},
  ...
  gambitSchema.pre("save", function (next) {
    ...
    if (self.input) {
      ...
      var language = langdetect.detect(input);
      // We want to convert the input into a trigger.
      regexreply.parse(Utils.quotemeta(input, true), language, facts, function (trigger) {
