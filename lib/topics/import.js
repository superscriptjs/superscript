  ...
  return function(path, callback) {
    ...
    async.each(Object.keys(data.gTopics), eachTopicItor, function(){
      ...
      var eachTopicPrevItor = function(topicName, next) {
        var eachGambitSetPrevItor = function(replyId, next2) {
          Reply.findOne({id:replyId}, function(err, reply) {
            ...
            if (reply) {
              ...
              async.eachSeries(Object.keys(gambitSet), eachGambitPrevItor, function(){
                next2();
              });
        ...
        async.eachSeries(Object.keys(data.gPrevTopics[topicName]), eachGambitSetPrevItor, function(){
          next();
        });
      };

      async.eachSeries(Object.keys(data.gPrevTopics), eachTopicPrevItor, function(){
        debug('All topics processed.');
        callback(null, "done");
      });
    });
  };
