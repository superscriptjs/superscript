...
module.exports = function (factSystem, Topic, Gambit, Reply) {
  ...
  var _rawToGambitData = function (gambitId, itemData) {
    ...
    var gambitData = {
      ...
      language: itemData.options.language,
      isDisordered: itemData.options.isDisordered,
      disorderedData: itemData.options.disorderedData
    };
  ...
  var importData = function (data, callback, flushTopics, preserveRandom) {
    ...
    // progress bar
    if (debug.name == 'disabled') {
      var progressLength = _.size(data.gambits) + _.size(data.replys); 
      var title;
      var progressBar = new progress('  importing [:bar] :percent eta :etas', {
          complete: '=',
          incomplete: '-',
          width: 20,
          total: progressLength
      });
    }
    ...
    async.eachSeries(Object.keys(data.topics), eachTopicItor, function () {
      ...
      async.eachSeries(_.uniq(gambitsWithConversation), function (gambitId, finish) {
        ...
        async.eachSeries(Object.keys(data.convos), convoItor, function() {
          debug.verbose("All Conditions Added");
          callback(null, "done");
        }); // end
