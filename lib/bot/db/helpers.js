...
var debug = (0, _debugLevels2.default)('SS:Common');

var _walkReplyParent = function _walkReplyParent(db, tenantId, replyId, replyIds, cb) {
  db.model('Reply').byTenant(tenantId).findById(replyId).populate('parent').exec(function (err, reply) {
    if (err) {
      debug.error(err);
    }
...
var _walkGambitParent = function _walkGambitParent(db, tenantId, gambitId, gambitIds, cb) {
  db.model('Gambit').byTenant(tenantId).findOne({ _id: gambitId }).populate('parent').exec(function (err, gambit) {
    if (err) {
      console.log(err);
    }
...
var findMatchingGambitsForMessage = function findMatchingGambitsForMessage(db, tenantId, type, id, message, options, callback) {
  ...
  if (type === 'topic') {
    ...
    db.model('Topic').byTenant(tenantId).findOne({ _id: id }, 'gambits').populate({ path: 'gambits' }).exec(execHandle);
  } else if (type === 'reply') {
    options.topic = 'reply';
    debug.verbose('Looking back at Conversation', id);
    db.model('Reply').byTenant(tenantId).findOne({ _id: id }, 'gambits').populate({ path: 'gambits' }).exec(execHandle);
  } else {
