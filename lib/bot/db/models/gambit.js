...
var createGambitModel = function createGambitModel(db) {
  ...
  gambitSchema.methods.addReply = function (replyData, callback) {
    ...
    var Reply = db.model('Reply').byTenant(this.getTenantId());
  ...
  gambitSchema.methods.clearReplies = function (callback) {
    ...
    var clearReply = function clearReply(replyId, cb) {
      ...
      db.model('Reply').byTenant(this.getTenantId()).remove({ _id: replyId }, function (err) {
        if (err) {
          console.log(err);
        }
  ...
  gambitSchema.methods.getRootTopic = function (cb) {
    ...
    if (!this.parent) {
      db.model('Topic').byTenant(this.getTenantId()).findOne({ gambits: { $in: [this._id] } }).exec(function (err, doc) {
        cb(err, doc.name);
      });
    } else {
      _helpers2.default.walkGambitParent(db, this.getTenantId(), this._id, function (err, gambits) {
        if (gambits.length !== 0) {
          db.model('Topic').byTenant(_this3.getTenantId()).findOne({ gambits: { $in: [gambits.pop()] } }).exec(function (err, topic) {
            cb(null, topic.name);
          });
        } else {
          cb(null, 'random');
