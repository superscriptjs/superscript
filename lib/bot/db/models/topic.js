...
var createTopicModel = function createTopicModel(db) {
  ...
  topicSchema.methods.createGambit = function (gambitData, callback) {
    ...
    var Gambit = db.model('Gambit').byTenant(this.getTenantId());
  ...
  topicSchema.methods.sortGambits = function (callback) {
    ...
    var expandReorder = function expandReorder(gambitId, cb) {
      db.model('Gambit').byTenant(_this2.getTenantId()).findById(gambitId, function (err, gambit) {
        if (err) {
          console.log(err);
        }
        cb(null, gambit);
  ...
  topicSchema.methods.doesMatch = function (message, options, cb) {
    ...
    db.model('Topic').byTenant(this.getTenantId()).findOne({ name: this.name }, 'gambits').populate('gambits').exec(function (err, mgambits) {
      if (err) {
        debug.error(err);
      }
  ...
  topicSchema.methods.clearGambits = function (callback) {
    ...
    var clearGambit = function clearGambit(gambitId, cb) {
      ...
      db.model('Gambit').byTenant(_this3.getTenantId()).findById(gambitId, function (err, gambit) {
        if (err) {
          debug.error(err);
        }

        gambit.clearReplies(function () {
          db.model('Gambit').byTenant(_this3.getTenantId()).remove({ _id: gambitId }, function (err) {
            if (err) {
              debug.error(err);
            }
  ...
  topicSchema.statics.findTriggerByTrigger = function (input, callback) {
    db.model('Gambit').byTenant(this.getTenantId()).findOne({ input: input }).exec(callback);
  };
        ...
        if (delta <= 1000 * 300) {
          (function () {
            ...
            if (clearConversation === true) {
              ...
              db.model('Reply').byTenant(_this4.getTenantId()).find({ _id: { $in: lastReply.replyIds } }).exec(function (err, replies) {
                if (err) {
                  console.error(err);
                }
                if (replies === []) {
