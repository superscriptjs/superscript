...
var historyLookup = module.exports = function(user, options) {
    ...
    if (pobj !== undefined) {
      ...
      if (options.money == true && options.nouns) {
        if (pobj.numbers.length != 0) {
          ...
          var t = [];
          if (_.any(pobj.taggedWords, function(item, key){ return item[1] == "$" || item[0] == "quid" || item[0] == "pounds" || item[0] == "dollars" || item[0] == "bucks" || item[0] == "cost"})) {
            t.push(pobj);

            // Now filter out the nouns
            for (var n = 0; n < t.length; n++) {
              var nn = _.any(t[n].nouns, function(item) {
        ...
        if (pobj.numbers.length != 0) {
          if (_.any(pobj.taggedWords, function(item, key){ return item[1] == "$" || item[0] == "quid" || item[0] == "pounds" || item[0] == "dollars" || item[0] == "bucks"})) {
            canidates.push(pobj);
          }
        }
        ...
        if (_.isArray(options.nouns)) {
          ...
          var nn = _.any(pobj.nouns, function(item){
        ...
        if (_.isArray(options.names)) {
          var nn = _.any(pobj.names, function(item){
        ...
        if (_.isArray(options.adjectives)) {
          ...
          var nn = _.any(pobj.adjectives, function(item){
