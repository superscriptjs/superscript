...
var matchItorHandle = function(user, message) {
  return function(match, callback) {
    ...
    // lets do it here
    topicSystem.gambit.findById(match.trigger._id).populate('replies').exec(function(err, triggerExpanded){
  ...
  async.filter(replies, filterHandle, function(filteredReplies){
    ...
    filterRepliesBySeen(filteredReplies, user,  function afterFilterRepliesBySeen(err, reply){
      ...
      if (err) {
        ...
        processTags(reply, user, options, function afterProcessTags(err, processedReplyString, props, replyObj){

          debug("err, reply", err, processedReplyString, props, replyObj);
