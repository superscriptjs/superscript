      ...
      if (sorted["thats"][top]) {
        ...
        if (lastReply) {
          ...
          var eachPrevTrigItor = function(trig, cb1) {
            ...
            // var gambit = thats[topic][trig];
            // var match = false;
            // for (item in gambit) {
            //   var botside = gambit[item].trigger;

            //   debug("eachPrevGam", botside);
            //   debug("Try to match lastReply '" + lastReply + "' against -- " + item + " -- " + trig + " (" + botside + ")");
            ...
            //     var humanside = thats[topic][trig][subtrig].trigger;
            ...
            //   debug("We have a Match, now look at ", sorted["that_trig"][top][trig])
            //   async.eachSeries(sorted["that_trig"][top][trig], subTrigItor, function(err, res){
            //     cb1(null);
            //   });
