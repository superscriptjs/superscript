    ...
    if (topicData.type === "TOPIC") {
      topSystem.topic.findOne({_id: topicData.id})
        ...
        .exec(function (err, topic) {
          ...
          topic.checkConditions(message, user, gPlugins, gScope, function(err, matches) {
...
var afterHandle = function (user, callback) {
  ...
  return function (reply, replySet) {
    
    var s = "";
    // Remove null values from set.
    // If we have a replySet this is because of force continue
    replySet = _.compact(replySet);
    if (!_.isEmpty(replySet)) {
      for (var i = 0; i < replySet.length; i++) {
        debug("Concat Replies", replySet[i]);
        // TODO: (maybe) Merge the star data and message Props
        // For now, just mash the replyString together.
        if (replySet[i] && replySet[i].reply && replySet[i].reply.reply) {
          s += replySet[i].reply.reply + " ";
        }
      }
      if (reply) {
        reply.string = s += reply.string  
      }
      ...
      if (err) {
        ...
        processTags.replies(reply, user, options, function (err2, replyObj, props) {
          ...
          if (!_.isEmpty(replyObj)) {
            ...
            } else if (replyObj.breakBit === true || cbdata.string !== "" || cbdata.subReplies.length !== 0) {
  ...
  topicSystem.topic.findPendingTopicsForUser(user, message, function (err, aTopics) {
    ...
    async.mapSeries(
      ...
      function topicCompleteHandle(err2, results) {
        ...
        // Was `eachSeries` 
        async.mapSeries(matches, matchItorHandle(user, message), afterHandle(user, callback));
      }
    );
  });
};
