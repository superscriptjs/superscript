...
module.exports = getreply = function(options, callback) {
	...
	if (!foundMatch) {
		...
		// debug("DUMP", sortedSet["topics"][topic])
		async.eachSeries(sortedSet["topics"][topic], eachGambitHandle, function(err, res){
			...
			var afterHandle = function() {
				...
				if (context == "begin") {
					...
					debug("Not in begin, check for more tags", message, reply);
					return processTags(user, message, reply, stars, thatstars, step, plugins, function(err, reply){
						if (err) {
							var options = {
								user: user, 
								topics: topics, 
			...
			if (matched) {
				if (matched["redirect"]) {
					...
					processTags(user, message, matched["redirect"], stars, thatstars, step, plugins, function(err, redirect) {
