...
var getreply = function(options, callback) {
  ...
  topicSystem.findPendingTopicsForUser(user, message, function(err, aTopics){
    ...
    for (var i = 0; i <= 10; i++) {
      ...
      if (topicItem !== undefined) {
        ...
        if (!_.isNull(pastGambit) && !_.isNull(pastInput)) {
          ...
          // debug("---", pastGambit.replyId, repIndex)
          // debug("----", pastInput.triggerId, inputID)
  ...
  async.filter(replies, filterHandle, function(filteredReplies){
    ...
    filterRepliesBySeen(filteredReplies, user,  function afterFilterRepliesBySeen(err, reply){
      ...
      if (err) {
        ...
        processTags(reply, user, options, function afterProcessTags(err, processedReply){
          debug("err, reply", err, processedReply);
          if (processedReply === "") {
            callback();
          } else {
            callback({string: processedReply, id: reply.crc}, null);
          }
        }); 
      }
    });
