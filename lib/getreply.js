		...
		async.eachSeries(sortedSet["topics"][topic], eachGambitHandle, function(err, res){
			...
			var afterHandle = function() {
				...
				if (context == "begin") {
					...
					return processTags(user, message, reply, stars, thatstars, step, plugins, topicFlags,  function(err, reply){
						if (err) {
			...
			if (matched) {
				if (matched["redirect"]) {
					...
					processTags(user, message, matched["redirect"], stars, thatstars, step, plugins, topicFlags, function(err, redirect) {
