...
var topicItorHandle = function(topicSystem, message, user) {
  ...
  return function(topicName, callback) {
    topicSystem.topic.findOne({name:topicName}).populate('gambits').exec(function(err, topic){
      if (topic) {
...
var filterRepliesBySeen = function(filteredResults, user, callback) {
  ...
  for (var j = 0; j < filteredResults.length; j++) {
    ...
    var topicName = filteredResults[j].topic;
    var currentTopic = topicSystem.findTopicByName(topicName);
  ...
  async.filter(replies, filterHandle, function(filteredReplies){
    ...
    filterRepliesBySeen(filteredReplies, user,  function afterFilterRepliesBySeen(err, reply){
      ...
      if (err) {
        ...
        processTags(reply, user, options, function afterProcessTags(err, processedReply, props){
          debug("err, reply", err, processedReply, props);
          if (processedReply === "") {
            callback();
          } else {
            callback({string: processedReply, id: reply.crc, props: props}, null);
          }
        }); 
      }
    });
