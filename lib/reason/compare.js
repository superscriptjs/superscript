...
module.exports = function(message, user, facts, cb) {
	...
	if (message.compareWords.length == 2 && message.compareWords[0] === message.compareWords[1] ) {
		...
		var ind = message.words.indexOf(message.compareWords[0]);
		...
		var oppisiteWord = facts.query("direct_sv", lemCmpWord, "opposite");
		var relation = memory.domain.Relation.fetch(lemCmpWord);
		
		// Lets create a new Relation
		if (!relation && oppisiteWord) {
			memory.createRelation(lemCmpWord, oppisiteWord, true);
		}

		if (message.nouns.length == 2) {
			suggestedReply = message.nouns[0] + " is " + message.compareWords[0] + " than " + message.nouns[1] + ".";

			console.log("FAIL HERE", lemCmpWord);
			memory.createfact(message.nouns[0], lemCmpWord, message.nouns[1]);
		} else {
			debug("We have a complicated Compare phrase")
			// given: John is older than Mary, and Mary is older than Sarah.
			// We already know the JJR is the same
			if (message.nouns.length == 4) {
				memory.createfact(message.nouns[0], lemCmpWord, message.nouns[1], true);
				memory.createfact(message.nouns[2], lemCmpWord, message.nouns[3], true);
			}
		}
		...
		if (_.contains(['bigger','larger'], message.compareWords[0])) {
			...
			if (speedResult != null) {
				...
				if (message.isQuestion) {
					suggestedReply = "I'm nounsot sure which is faster, why don't you tell me.";
				} else {
					...
					if (speedResult1 == true) {	
						suggestedReply = obj.nouns[0] + " is faster than " + obj.nouns[1] + ", if memory serves.";
					} else {
			...
			if (message.isQuestion) {
				...
				var cmpResult1 = memory.query("direct_svo", message.nouns[0], lemCmpWord, message.nouns[1]);
				var cmpResult2 = memory.query("direct_svo", message.nouns[1], lemCmpWord, message.nouns[0]);
