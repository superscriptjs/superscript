...
Message.prototype.lemma = function() {
  ...
  return _.map(that.taggedWords, function(item, key){
    var w, lw, word, f;
    w = item[0].toLowerCase();
    lw = lemmerEng.lemmatize(w);

    if (_.isEmpty(lw)) {
      word = w;
    } else if (lw.length == 1) {
      word = lw[0].text.toLowerCase();
    } else if (lw.length > 1) {

      var res = lw.reduce(function(a, b){return (a.text === b.text)?a:false;});
      if (res !== false) {
        word = lw[0].text.toLowerCase();
      } else {
        word = null;
        // we need to check the POS
        if (Utils._isTag(item[1], 'verbs')) {
          f = lw.filter(function(a){return (a.partOfSpeech === 'VERB');});
          word = (f.length == 1)?f[0].text.toLowerCase(): null;
        }

        if (Utils._isTag(item[1], 'nouns')) {
          f = lw.filter(function(a){return (a.partOfSpeech === 'NOUN');});
          word = (f.length == 1)?f[0].text.toLowerCase(): null;
        }

        if (!word) {
          var left = lw.filter(function(a){return (a.partOfSpeech !== 'NOUN' && a.partOfSpeech !== 'VERB');});
          if (left.length == 1) {
            word = left[0].text.toLowerCase();
          } else {
            word = w;
          }
        }
