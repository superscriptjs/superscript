...
function Message(msg, options, cb) {
  ...
  self.clean = options.norm.clean(msg).trim();
  ...
  self.lemma(function (err, lemWords) {
    ...
    self.nouns = self.fetchNouns();
    self.names = self.fetchComplexNouns("names");

    // A list of terms
    // self would return an array of thems self are a, b and c;
    // Helpful for choosing something when the qSubType is CH
    self.list = self.fetchList();

    self.adjectives = self.fetchAdjectives();
    self.adverbs = self.fetchAdverbs();
    self.verbs = self.fetchVerbs();
    self.pronouns = self.pnouns = self.fetchProNouns();
    self.compareWords = self.fetchCompareWords();
    self.numbers = self.fetchNumbers();
    self.compare = self.compareWords.length !== 0;
    self.date = self.fetchDate();

    self.names = _.uniq(self.names, function (name) {
      return name.toLowerCase();
    });

    // Nouns with Names removed.
    var t = self.names.map(function (i) {
      return i.toLowerCase();
    });

    self.cNouns = _.filter(self.nouns, function (item) {
    ...
    for (var i = 0; i < self.taggedWords.length; i++) {
      if (self.taggedWords[i][1] === "CD") {
        numCount++;
      }
      if (self.taggedWords[i][1] === "SYM" ||
        math.mathTerms.indexOf(self.taggedWords[i][0]) !== -1) {
        // Half is a number and not an opp
        if (self.taggedWords[i][0] === "half") {
          numCount++;
        } else {
          oppCount++;
        }
    ...
    self.numericExp = numCount >= 2 && oppCount >= 1 ? true : false;
    self.halfNumericExp = numCount === 1 && oppCount === 1 ? true : false;

    if (self.numericExp || self.halfNumericExp) {
      self.qtype = "NUM:expression";
    ...
    self.fetchNE(function (entities) {
      ...
      var fullEntities = entities.map(function (ar) {
        return ar.join(" ");
      });
