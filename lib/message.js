...
function Message(msg, options, cb) {
  ...
  that.lemma(function(err, lemWords){
    
    that.lemWords = lemWords;
    ...
    that.nouns = that.fetchNouns();
    that.names = that.fetchComplexNouns("names");

    // A list of terms
    // that would return an array of thems that are a, b and c;
    // Helpful for choosing something when the qSubType is CH
    that.list = that.fetchList();

    that.adjectives = that.fetchAdjectives();
    that.adverbs = that.fetchAdverbs();
    that.verbs = that.fetchVerbs();
    that.pronouns = that.pnouns = that.fetchProNouns();
    that.compareWords = that.fetchCompareWords();
    that.numbers = that.fetchNumbers();
    that.compare = (that.compareWords.length !== 0);
    that.date = that.fetchDate();
...
var pennToWordnet = function(pennTag) {
  if (S(pennTag).startsWith('J')) {
    return "a";
  } else if (S(pennTag).startsWith('V')) {
    return "v";
  } else if (S(pennTag).startsWith('N')) {
    return "n";
  } else if (S(pennTag).startsWith('R')) {
    return "r";
  } else {
    return null;
  }
...
Message.prototype.lemma = function(callback) {
  ...
  async.map(this.taggedWords, itor, function(err, transformed){
    var res = _.map(_.flatten(transformed), function(a){ return a.split("#")[0]; });
    callback(err, res);
  });
};
...
Message.prototype.fetchDate = function() {
  ...
  if (that.clean.match(re)) {
    var m = that.clean.match(re);
    debug("Date", m);
