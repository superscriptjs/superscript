...
var Message = function (msg, options, cb) {
  ...
  var self = this;
  var extraScope = options.extraScope;

  // if there is an empty reply, let it through if the bot was also provided with extraScope
  if (!msg && !extraScope) {
    debug.verbose("Callback Early, empty msg, and no extraScope");
    return cb({});
  }
