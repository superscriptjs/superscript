...
User.prototype.set = function(key, value) {
  ...
  that.memory.get({subject:key, predicate: that.name}, function(err, results) {
    if (!_.isEmpty(results)) {
      that.memory.del(results[0], function(){
        that.memory.put({subject:key, predicate: that.name, object:value});  
      })
    } else {
      that.memory.put({subject:key,predicate: that.name, object:value});  
    }
...
User.prototype.get = function(key, cb) {
  debug("getVar", key);
  this.memory.get({subject:key, predicate: this.name}, function(err, res){
    cb(err, res[0].object);
  });
}
