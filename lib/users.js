...
Users = {
  ...
  findOrCreate: function(userName, facts, cb) {
    var that = this;

    // First load.
    var ut = {subject:userName, predicate:"saved_data"};
    facts.db.get(ut, function(err, frozenUser) {
      // This is a new user
      if (_.isEmpty(frozenUser)) {
        ut.object = userName;
        facts.db.put(ut, function() {
          debug("Saving", ut);
          that._users[userName] = new User(userName, facts);          
          cb(null, that._users[userName]);
        });
      } else {
        var userId = frozenUser[0].object;
        if (that._users[userId]) {
          debug("Loading");
          var user = that._users[userId];
          user.memory = facts.createUserDB(userId);
          cb(null, that._users[userId]);
        } else {
          debug("Re-Loading", userId);
          that._users[userId] = new User(userId, facts);
          cb(null, that._users[userId]);          
        }
      }
    });
  ...
  connect: function(userName, facts, cb) {

    this.findOrCreate(userName, facts, function(err, user){
      user.status = 1;
...
function User(userId, facts) {
  ...
  this.pendingTopic;
  // this.memory = facts.createUserDB(userId);
...
User.prototype.getLastReply = function() {
  debug("getLastMessage");
  return this['__history__']["reply"][0];
}
