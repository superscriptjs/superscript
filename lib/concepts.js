...
var stopwords = ["for", "use", "an", "if", "of", "to", "the", "is", "a", "I", "i", "are", "and", "who", "what", "where", "when","how", "would", "which", "or", "do", "my", "bob"];
...
SuperConcept.prototype.highLevelLookup = function(msg, cb) {
  ...
  var itor = function(item, cb) {
    ...
    for (var i = 0; i < that.concepts.length; i++) {
      ...
      if (c) {
        var dsv = that.query("direct_sv", c.id, "isa");
        if (_.isEmpty(dsv) && _.contains(that.concepts[i].highLevelConcepts, c.id)) {
          dsv = c.id
        }
        m.push({ 'hlc' : dsv, 'word': c.id});
      }
    }
    cb(null, _.unique(m));
  }
  ...
  async.map(words2, itor, function(err, res){
    cb(null, _.unique(_.flatten(res), function(con){ return con.word }));
  });
}
...
exports.readFile = readFile = function(file, domain, cb) {
  ...
  var findOrCreateConcept = function(term, concept) {
    ...
    // If the concept already exists, but not part of this collection
    if (fetched && fetched.id != concept) {
      return fetched;
...
exports.readFiles = function(files, callback) {
  ...
  var fileHandle = function(domain) {
    return function(file, next) {
      readFile(file, domain, function(con) {
        next(null, con);
      });
    }
  }

  async.mapSeries(files, fileHandle(expert.Domain()), function(err, conceptMap){
