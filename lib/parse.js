  ...
  var itor = function(line, next) {
      ...
      if (lookahead.length != 0) {
        ...
        if (cmd == 'H' || cmd == '+' || cmd == '?') {
          if (lookCmd == '%') {
            ...
            debug("Creating CRC", lookahead, crcPrevTrigger);
    ...
    switch(cmd) {
      ...
      case "+":
        ...
        regexreply.parse(line, function(regexp) {
          ...
          if (isPrevious.length > 0) {
            debug("IS PREV", isPrevious, crcPrevTrigger, regexp);
      ...
      case "-":
        ...
        if (isPrevious.length > 0) {
          debug("XX---", crcPrevTrigger, ontrig, line);
