...
var replies = function (replyObj, user, options, callback) {
  ...
  if (clearMatch) {
    debug("Adding Clear Conversation Bit");
  ...
  if (mbitMatch) {
    debug("Adding CONTINUE Conversation Bit");
  ...
  if (mbit2Match) {
    debug("Adding END Conversation Bit");
      ...
      return function(err, finalReply) {
        ...
        if (msgStateMatch && finalReply.indexOf("delay") === -1) {
          for(var i = 0; i < msgStateMatch.length; i++) {
            ...
            debug("Found Conversation State", stateObj);
