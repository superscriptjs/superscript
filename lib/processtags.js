...
module.exports = function (options, callback) {
	...
	if (currentFlags.indexOf("keep") == -1 && step == 0) {
		for (var i = 0; i <= 10; i++) {
	...
	function wordnetReplace(match, sym, word, p3, offset, done) {
		wordnet.lookup(word, sym, function(err, words){
			...
			var resp = getRandomInt(0, words.length - 1);
				...
				function (cb) {
					...
					if (plugins[obj]) {
						...
						args.push(function customFunctionHandle(err, subreply){
							// TODO Change this RegEx to actually match functions with OPTIONS!
							reply = reply.replace(new RegExp("\\^" + Utils.quotemeta(obj) + "\\(\\)", "ig"), subreply);
