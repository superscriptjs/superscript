...
exports.replies = function (replyObj, user, options, callback) {
  ...
  var afterHandle = function(err, finalReply) {
    if (err) {
      console.log(err);
    }
    ...
    // The topicSetter returns an array with the reply and topic
    var parsedTopicArr = processHelpers.topicSetter(replyString);
    var newtopic = (parsedTopicArr[1] !== "") ? parsedTopicArr[1] : null;
    replyString = parsedTopicArr[0];
    ...
    callback(err, replyString, globalScope.message.props);
  };
  ...
  replace(reply, /(~)(\w[\w]+)/g, processHelpers.wordnetReplace, function (err, newReply) {
    ...
    if (newReply.match(/\^(\w+)\(([\w<>,\s]*)\)/)) {
      processHelpers.parseCustomFunctions(newReply, plugins, globalScope, afterHandle);
    } else {
      afterHandle(null, newReply);
    }
  });
};
