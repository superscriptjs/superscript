...
module.exports = function (options, callback) {
  ...
  // This has been moved to getReply
  ...
  replace(reply, /(~)(\w[\w]+)/g, wordnetReplace, function(err, reply) {
    ...
    if (match = reply.match(/\^(\w+)\(([\w<>,\s]*)\)/)) {
      ...
      parseCustomFunctions(reply, plugins, scope, function(reply) {
...
var parseCustomFunctions = function(reply, plugins, scope, callback ) {
  ...
  // This matches the Redirect finder above
    ...
    return async.whilst(
      ...
      function (cb) {
        ...
        if (plugins[pluginName]) {
          ...
          args.push(function customFunctionHandle(err, subreply) {
            ...
            match = false;
            // var re = new RegExp("\\^" + Utils.quotemeta(pluginName) + "\\(\\s*[a-z0-9,\\s~<>]*\\s*\\\)", "ig");
      ...
      function() {
        debug("Calling back from Function with ", reply)
        callback(reply)  
      }
