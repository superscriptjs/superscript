...
var _searchMiniTopic = function(cursor, cmd, instructions) {
    ...
    for (var i = cursor; i < instructions.length; i++) {
      ...
      if (lookahead.length != 0) {
        ...
        if (cmd === '!') {
          if (lookCmd === '^') result.line = "<crlf>" + lookahead;
          continue;
        }
    ...
    var instructionsItor = function(raw, nextInstruction) {
      ...
      switch(cmd) {
        ...
        case "+":
          ...
          debug('isPrevious: ' + miniTopic.isPrevious);
          ...
          regexreply.parse(line, factSystem, function(regexp) {
            ...
            gambit = _initGambitTree(topicName, idTrigger, regexp, line, line, trigOptions);
            if(_.size(gambit) > 0) {
              currentTrigger = idTrigger;
        ...
        case '>':
          ...
          if (type === "pre" || type === "post") {
            ...
            currentTrigger = null;
            topicName  = name;
            topics[name].flags = topics[name].flags.concat(flags.join(','));
          } else {
        ...
        case "*":
          ...
          var idTrigger = Utils.genId();
          gambit = _initGambitTree(topicName, idTrigger, line, line, line, options, 'botTopics');
          if(_.size(gambit) > 0) {
            //currentTrigger = idTrigger;
            gambits = _.extend(gambits, gambit);
          }
...
var _initGambitTree = function (topicName, idTrigger, regexp, input, raw, options, kindTopic) {
  ...
  if(kindTopic === 'botTopics') {
    ...
    gambit[idTrigger].trigger = regexp;
  }
  return gambit;
};