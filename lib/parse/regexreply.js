...
exports.parse = function (regexp, facts, callback) {
  ...
  function nWidthStarMatch(match, p1, offset, string){
    if (p1) debug("E WIDTH STAR", p1, match);
    return "<"+parseInt(p1)+"ewidthstar>";
  }

  // Step 2 nWidthStar
  // (\s?(?:[\w]*\s?){0,n})
  function varWidthStarReplace(match, p1, offset, string){
    if (p1) debug("V WIDTH STAR", p1, match);
    var num = parseInt(p1.replace("~",""));
    return "<"+num+"vwidthstar>";
  }

  // Step 3 mix-maxWidthStar
  function mmWidthStarReplace(match, p1, offset, string){
  ...
  regexp = regexp.replace(/\*(~[0-9]{1,2}\s?)/g, varWidthStarReplace);  // Convert *~n into multi word VARIABLE match
  ...
  replace(regexp, /(~)(\w[\w]+)/g, wordnetReplace, function(err, result) {
    regexp = result;
      ...
      // mmvWidthStar
      function mmWidthReplace(match, p1, offset, string){
        var parts = p1.split(",");
